...
def display_data(limit = 20):
    ...
    row = session.execute("SELECT * FROM news_articles.train").all()
    print("="*20)
    count = 0
    for r in row:
        count += 1
...
def delete_query(id_=0):
    ...
    if id_ == -1:
        query_delete_one = "DELETE FROM news_articles.train"
    else:
        query_delete_one = "DELETE FROM news_articles.train WHERE article_id = {};".format(id_)
